/**
 * Covers the hand-written facade in src/pay.ts: which transports get
...
import { Pay, type ConfigurationPay } from '../../src/pay';
...
describe('Pay', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Pay({} as ConfigurationPay);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Pay({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(PAY_REST_API_PROD_URL);
        });
