/**
 * Covers the hand-written facade in src/gift-card.ts: which transports get
...
import { GiftCard, type ConfigurationGiftCard } from '../../src/gift-card';
...
describe('GiftCard', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new GiftCard({} as ConfigurationGiftCard);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new GiftCard({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(GIFT_CARD_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new GiftCard({
