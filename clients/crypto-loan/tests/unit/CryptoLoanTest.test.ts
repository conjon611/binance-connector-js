/**
 * Covers the hand-written facade in src/crypto-loan.ts: which transports get
 * constructed, the production URL defaults, and User-Agent injection.
 */

import { platform, arch } from 'os';
import { expect, describe, it } from '@jest/globals';
import { CRYPTO_LOAN_REST_API_PROD_URL, ConfigurationRestAPI } from '@binance/common';
import { name, version } from '../../package.json';
import { CryptoLoan, type ConfigurationCryptoLoan } from '../../src/crypto-loan';
...
describe('CryptoLoan', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new CryptoLoan({} as ConfigurationCryptoLoan);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new CryptoLoan({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(CRYPTO_LOAN_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new CryptoLoan({
