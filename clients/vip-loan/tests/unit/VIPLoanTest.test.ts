/**
 * Covers the hand-written facade in src/vip-loan.ts: which transports get
...
import { VIPLoan, type ConfigurationVIPLoan } from '../../src/vip-loan';
...
describe('VIPLoan', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new VIPLoan({} as ConfigurationVIPLoan);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new VIPLoan({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(VIP_LOAN_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new VIPLoan({
