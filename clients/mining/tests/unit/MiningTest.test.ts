/**
 * Covers the hand-written facade in src/mining.ts: which transports get
...
import { Mining, type ConfigurationMining } from '../../src/mining';
...
describe('Mining', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Mining({} as ConfigurationMining);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Mining({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(MINING_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new Mining({
