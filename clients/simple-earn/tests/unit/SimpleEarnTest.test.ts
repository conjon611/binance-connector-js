/**
 * Covers the hand-written facade in src/simple-earn.ts: which transports get
...
import { SimpleEarn, type ConfigurationSimpleEarn } from '../../src/simple-earn';
...
describe('SimpleEarn', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new SimpleEarn({} as ConfigurationSimpleEarn);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new SimpleEarn({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(SIMPLE_EARN_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new SimpleEarn({
