/**
 * Covers the hand-written facade in src/rebate.ts: which transports get
...
import { Rebate, type ConfigurationRebate } from '../../src/rebate';
...
describe('Rebate', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Rebate({} as ConfigurationRebate);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Rebate({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(REBATE_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new Rebate({
