/**
 * Covers the hand-written facade in src/convert.ts: which transports get
 * constructed, the production URL defaults, and User-Agent injection.
 */

import { platform, arch } from 'os';
import { expect, describe, it } from '@jest/globals';
import { CONVERT_REST_API_PROD_URL, ConfigurationRestAPI } from '@binance/common';
import { name, version } from '../../package.json';
import { Convert, type ConfigurationConvert } from '../../src/convert';
...
describe('Convert', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Convert({} as ConfigurationConvert);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Convert({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(CONVERT_REST_API_PROD_URL);
        });
