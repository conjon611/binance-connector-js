/**
 * Covers the hand-written facade in src/staking.ts: which transports get
...
import { Staking, type ConfigurationStaking } from '../../src/staking';
...
describe('Staking', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Staking({} as ConfigurationStaking);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Staking({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(STAKING_REST_API_PROD_URL);
        });
