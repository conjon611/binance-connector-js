/**
 * Covers the hand-written facade in src/spot.ts: which transports get
...
import { Spot, type ConfigurationSpot } from '../../src/spot';
...
describe('Spot', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Spot({} as ConfigurationSpot);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Spot({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(SPOT_REST_API_PROD_URL);
        });
    ...
    describe('WebSocket API transport', () => {
        ...
        it('should default wsURL to the production URL', () => {
            const client = new Spot({ configurationWebsocketAPI: { apiKey: 'test-api-key' } });

            expect(client.websocketAPI).toBeDefined();
            expect(configOf(client.websocketAPI).wsURL).toBe(SPOT_WS_API_PROD_URL);
        });
    ...
    describe('WebSocket Streams transport', () => {
        ...
        it('should default wsURL to the production URL', () => {
            ...
            expect(configOf(client.websocketStreams).wsURL).toBe(SPOT_WS_STREAMS_PROD_URL);
        });
