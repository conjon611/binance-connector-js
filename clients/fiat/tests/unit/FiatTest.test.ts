/**
 * Covers the hand-written facade in src/fiat.ts: which transports get
...
import { Fiat, type ConfigurationFiat } from '../../src/fiat';
...
describe('Fiat', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Fiat({} as ConfigurationFiat);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Fiat({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(FIAT_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new Fiat({
