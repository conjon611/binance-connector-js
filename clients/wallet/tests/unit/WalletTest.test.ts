/**
 * Covers the hand-written facade in src/wallet.ts: which transports get
...
import { Wallet, type ConfigurationWallet } from '../../src/wallet';
...
describe('Wallet', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new Wallet({} as ConfigurationWallet);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new Wallet({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(WALLET_REST_API_PROD_URL);
        });
