/**
 * Covers the hand-written facade in src/copy-trading.ts: which transports get
 * constructed, the production URL defaults, and User-Agent injection.
 */

import { platform, arch } from 'os';
import { expect, describe, it } from '@jest/globals';
import { COPY_TRADING_REST_API_PROD_URL, ConfigurationRestAPI } from '@binance/common';
import { name, version } from '../../package.json';
import { CopyTrading, type ConfigurationCopyTrading } from '../../src/copy-trading';
...
describe('CopyTrading', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new CopyTrading({} as ConfigurationCopyTrading);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new CopyTrading({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(COPY_TRADING_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new CopyTrading({
