/**
 * Covers the hand-written facade in src/sub-account.ts: which transports get
...
import { SubAccount, type ConfigurationSubAccount } from '../../src/sub-account';
...
describe('SubAccount', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new SubAccount({} as ConfigurationSubAccount);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new SubAccount({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(SUB_ACCOUNT_REST_API_PROD_URL);
        });

        it('should keep an explicitly supplied basePath', () => {
            const client = new SubAccount({
