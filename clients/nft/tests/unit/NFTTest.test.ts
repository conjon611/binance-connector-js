/**
 * Covers the hand-written facade in src/nft.ts: which transports get
...
import { NFT, type ConfigurationNFT } from '../../src/nft';
...
describe('NFT', () => {
    describe('REST API transport', () => {
        it('should not be constructed when no REST configuration is supplied', () => {
            const client = new NFT({} as ConfigurationNFT);

            expect(client.restAPI).toBeUndefined();
        });

        it('should default basePath to the production REST URL', () => {
            const client = new NFT({ configurationRestAPI: { apiKey: 'test-api-key' } });

            expect(client.restAPI).toBeDefined();
            expect(configOf(client.restAPI).basePath).toBe(NFT_REST_API_PROD_URL);
        });
